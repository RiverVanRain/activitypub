...
$anchor = elgg_extract('anchor', $vars, null);
...
$icon_material = elgg_extract('icon_material', $vars, '');
unset($vars['icon_material']);

if ($icon && !preg_match('/^</', $icon)) {
	$icon = elgg_view_icon($icon, [
		'class' => 'elgg-anchor-icon',
	]);
} else if ($icon_material && !preg_match('/^</', $icon_material)) {
	$icon = theme_view_icon_material($icon_material, [
		'class' => 'elgg-anchor-icon icon-0 elgg-icon',
	]);
}
