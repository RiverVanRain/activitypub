...
class LikedController {
	...
	public function __invoke(\Elgg\Request $request): \Elgg\Http\Response {
		...
		try {
			...
			elgg_log('Liked routing callback error: ' . $e->getMessage(), \Psr\Log\LogLevel::ERROR);
	...
	protected function getCollectionInfo(\ElggUser|\ElggGroup $entity): array {
		...
		$items_count = elgg_call(ELGG_IGNORE_ACCESS, function () use ($entity, $activity_object) {
		...
		$url = $entity instanceof \ElggUser ? elgg_generate_url('view:activitypub:user:liked', [
			'guid' => (int) $entity->guid,
		]) : elgg_generate_url('view:activitypub:group:liked', [
			'guid' => (int) $entity->guid,
		]);
