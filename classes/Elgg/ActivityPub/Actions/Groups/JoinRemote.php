...
class JoinRemote
{
    public function __invoke(\Elgg\Request $request)
    {
        ...
        if (!(bool) elgg()->activityPubUtility->checkDomain($actor, $remote_group)) {
            return elgg_error_response(elgg_echo('activitypub:group:join:error:domain'));
        }
        ...
        try {
            ...
            $activity->setMetadata('activity_type', 'Join');
            $activity->setMetadata('actor', elgg()->activityPubUtility->getActivityPubID($actor));
            $activity->setMetadata('activity_object', $remote_group);
            ...
            if (!$activity->save()) {
                if ((bool) elgg_get_plugin_setting('log_general_inbox_error', 'activitypub')) {
                    $this->log(elgg_echo('activitypub:outbox:follow:error', [(int) $actor->guid, $remote_group]));
                }
        ...
        return elgg_ok_response('', elgg_echo('activitypub:outbox:join:success'));
    }
