...
class Reader
{
        ...
        if (!empty($item['tag'])) {
            ...
            foreach ($tags as $tag) {
                ...
                if ($group instanceof \ElggGroup) {
                    ...
                    // Create Reaction
                    elseif (elgg_is_active_plugin('river') && (bool) elgg_get_plugin_setting('enable_group', 'river') && $group->isToolEnabled('river')) {
                        return elgg_call(ELGG_IGNORE_ACCESS, function () use ($user, $group, $item) {
    ...
    */
    ...
    public function buildMicrosubItem(ActivityPubActivity $activity)
    {
        ...
        } elseif ($activity->getActivityType() === 'Announce') {
            $item['repost-of'] = [$activity->getActivityObject()];
        } elseif ($activity->getActivityType() === 'Follow') {
            $item['content'] = (object) ['html' => 'This person is now following you!'];
        } elseif ($activity->getActivityType() === 'Create') {
